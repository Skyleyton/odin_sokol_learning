...
// Update things.
// change the name.
sim_game_state :: proc(gs: ^GameState) {
	...
	if key_down(app_state.input_state, .UP) {
		fmt.println("Go up")
	}
}
...
handle_to_entity :: proc(gs: ^GameState, handle: Entity_Handle) -> ^Entity {
...
entity_to_handle :: proc(gs: GameState, entity: Entity) -> Entity_Handle {
	return entity.id
}

entity_create :: proc(gs: ^GameState) -> ^Entity {
	spare_en: ^Entity
	for &en in gs.entities {
...
entity_destroy :: proc(gs: ^GameState, entity: ^Entity) {
	mem.set(entity, 0, size_of(Entity))
}

setup_player :: proc(en: ^Entity) {
	en.type = .PLAYER
	en.flags |= {.ALLOCATED} // On rajoute le flag.
}

// Input (Keyboard, mouse) things.
Key_code :: enum {
    INVALID          = 0,
    SPACE            = 32,
    APOSTROPHE       = 39,  /* ' */
